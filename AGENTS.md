...
- Prefer combat/team-win results shown visually (who wins / strength bands), not only raw damage numbers.
...
- Dead champions must be excluded from calculator imports (do not count their combat stats).
...
- Prefer dark Anthropic/Apple-like product-tool UI that feels integrated, not a collage of disconnected assets.
...
- Riot `stats_update` position cadence in processed feeds is ~1s (not 0.5s); full-resolution compressed timelines are served from `public/data/` (e.g. `fur_vs_g2_timeline.json`).
