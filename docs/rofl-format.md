...
| `rofl2_replication_timed_hp.py` | Done — multi-sample timed type-107 HP toward `rofl-trusted-hp-v1` + identity bind attempt + fuse dry-run (`npm run rofl:replication-timed-hp`) |
| `rofl2_create_hero_discover.py` | Done — live CreateHero/spawn discover; BR1 blocker `champion_not_structurally_decoded` (`npm run rofl:create-hero-discover`) |
| `rofl2_replication_combat_inventory.py` | Done — combat inventory only; `combat_wire_unproven` until index proof (`npm run rofl:replication-combat-inventory`) |
| `rofl2_ability_ranks_probe.py` | Done — ranks probe; `ability_ranks_wire_unproven` (`npm run rofl:ability-ranks-probe`) |
| `rofl_product_pipeline.py` | Done — Gate A→D orchestrator; fail-closed publish (`npm run rofl:product-pipeline`) |
