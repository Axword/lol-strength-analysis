...
1. **ROFL decrypt trust** — keep pushing HP / combat / ranks on real matches via CastSpellAns bind, PE combat wire, UpgradeSpellAns. Match-level `calculatorReady` stays false while any Replay API frame lacks honest `hpKnown` (current terminal: MonkeyKing first explicit `mMaxHP` ~77s on BR1-3264361042).
...
- Early-frame HP density for match-level `calculatorReady` (MonkeyKing / first explicit `mMaxHP`).
